#!/usr/bin/env python3
"""Fixed nested-calibration confirmation for 1m multi-horizon upside excursion."""
...
DEFAULT_OUTPUT_ROOT = Path(r"F:\rsio\rawseq_1m_upside_excursion_calibrated_confirmations")
...
TARGETS = [f"upside_event_0p5vol_h{h}m_fw240" for h in TARGET_HORIZONS]
...
def build_final_artifact(
    ...
    artifact = {
        "artifact_type": "rawseq_1m_upside_excursion_final_models",
...
def summarize_gates(primary_rows: list[dict[str, Any]], per_symbol_rows: list[dict[str, Any]], calibration_rows: list[dict[str, Any]], monotonicity_rows: list[dict[str, Any]], parity_rows: list[dict[str, Any]], feature_contract_pass: bool, target_contract_pass: bool, provenance_pass: bool) -> tuple[dict[str, Any], list[dict[str, Any]], str]:
    ...
    recommendation = (
        "frozen_upside_excursion_board_member_waiting_for_august_holdout"
        if all_pass
        else "upside_excursion_not_confirmed"
    )
    summary = {
...
def downside_member_comparison_rows(primary_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    rows.append(
        {
            ...
            "both_probabilities_elevated_fraction": math.nan,
            "both_probabilities_low_fraction": math.nan,
...
def write_august_contracts(freeze_dir: Path, candidate_hash: str, member_contract_hash: str) -> None:
    ...
    holdout = {
        "holdout_name": "august_2026_upside_excursion",
    ...
    write_json(freeze_dir / "upside_excursion_august_holdout_contract.json", holdout)
    write_json(freeze_dir / "upside_excursion_august_acceptance_rule.json", acceptance)
    write_json(freeze_dir / "upside_excursion_august_access_ledger.json", {"created_at": now_stamp(), **access_flags})
    write_json(freeze_dir / "upside_excursion_august_source_expectation.json", {"expected_complete_rows_per_symbol": 44640, "files_may_be_opened_after": "2026-09-01T00:00:00Z"})
...
def maybe_freeze(out_dir: Path, contract: dict[str, Any], feature_contract: dict[str, Any], target_contract: dict[str, Any], gate_summary: dict[str, Any], final_artifact: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if final_artifact.get("final_artifact_status") != "OK":
        ...
        return blocked
    freeze_dir = out_dir / "frozen_upside_excursion_board_member_challenger"
    ...
    member_contract = {
        "member": "upside_horizon_term_structure",
    ...
    packet = {
        ...
        "freeze_status": "frozen_upside_excursion_board_member_waiting_for_august_holdout",
        ...
        "board_role": "upside_horizon_term_structure",
...
def text_decision(path: Path, summary: dict[str, Any], contract: dict[str, Any]) -> None:
    lines = [
        "RAWSEQ 1M UPSIDE-EXCURSION CALIBRATED CONFIRMATION",
...
def main() -> int:
    ...
    source_root = env_path("RAWSEQ_UPSIDE_SOURCE_PATH", DEFAULT_SOURCE_PATH)
    output_root = env_path("RAWSEQ_UPSIDE_OUTPUT_ROOT", DEFAULT_OUTPUT_ROOT)
    symbols = [s.strip().upper() for s in os.getenv("RAWSEQ_UPSIDE_SYMBOLS", ",".join(DEFAULT_SYMBOLS)).split(",") if s.strip()]
    max_rows = int(os.getenv("RAWSEQ_UPSIDE_MAX_ROWS_PER_SYMBOL", "50000"))
    train_cap = int(os.getenv("RAWSEQ_UPSIDE_MAX_TRAIN_ROWS_PER_SYMBOL", "20000"))
    min_rows = int(os.getenv("RAWSEQ_UPSIDE_MIN_ROWS", "500"))
    out_dir = output_root / f"rawseq_1m_upside_excursion_calibrated_confirmation_{now_stamp()}"
    ...
    target_contract = {
        "target_family": "multi_horizon_upside_0p5vol",
        "target_names": TARGETS,
        "horizons_minutes": TARGET_HORIZONS,
        "volatility_window_minutes": 240,
        "threshold_vol_units": 0.5,
        "ordering": "p_1m <= p_2m <= p_4m <= p_8m",
        "target_formula": "P(maximum upside excursion exceeds 0.5 trailing-volatility units within horizon)",
        "development_cutoff_iso": "2026-05-31T23:59:00Z",
    }
    target_contract["target_contract_hash"] = stable_hash(target_contract)
    expected_targets = ["upside_event_0p5vol_h1m_fw240", "upside_event_0p5vol_h2m_fw240", "upside_event_0p5vol_h4m_fw240", "upside_event_0p5vol_h8m_fw240"]
    missing_target_cols = {
    ...
    write_csv(out_dir / "downside_member_comparison.csv", downside_member_comparison_rows(primary_rows))
