...
ALLOW_SYNTHETIC_INPUT = os.getenv("SIM_CALIBRATION_ALLOW_SYNTHETIC_INPUT", "false").strip().lower() in {
    "1",
    "true",
    "yes",
    "y",
...
SYNTHETIC_PATH_MARKERS = {"simulated", "synthetic", "simulated_rolling", "simulated_runs", "sim_holdout"}
...
def path_looks_synthetic(path):
    text = str(path).lower()
    parts = {part.lower() for part in Path(path).parts}
    return any(marker in parts for marker in SYNTHETIC_PATH_MARKERS) or "simulated" in text or "synthetic" in text


def should_use_source_path(path):
    return ALLOW_SYNTHETIC_INPUT or not path_looks_synthetic(path)
...
def discover_input_files():
    if SOURCE_PATH_ENV:
        ...
        return [path] if path.exists() and should_use_source_path(path) else []
    ...
    return [path for path in candidates if should_use_source_path(path)]
...
def normalize_candle_frame(frame, source_path):
    ...
    source_kind_column = lower_columns.get("source_kind")
    if source_kind_column is not None and not ALLOW_SYNTHETIC_INPUT:
        source_kind = frame[source_kind_column].fillna("").astype(str).str.strip().str.lower()
        frame = frame[source_kind != "synthetic"].copy()
        if len(frame) == 0:
            return pd.DataFrame()
        lower_columns = {str(column).strip().lower(): column for column in frame.columns}
...
def atomic_write_json(payload, path):
    tiny_io.safe_write_json_atomic(payload, path)
