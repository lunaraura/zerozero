...
def read_csv(path):
    if not path.exists():
        return pd.DataFrame()
    try:
        return tiny_io.read_csv_required(path, "simulated run flow", prefer_parquet=False)
...
def combine_kind(runs, suffix):
    ...
    for run in runs:
        ...
        if "source_scenario" not in frame.columns:
            frame["source_scenario"] = run["scenario"]
        else:
            frame["source_scenario"] = frame["source_scenario"].fillna(run["scenario"]).replace("", run["scenario"])
        ...
        if "source_kind" not in frame.columns:
            ...
            frame["source_kind"] = frame["source_kind"].fillna(SOURCE_KIND).replace("", SOURCE_KIND)

        if "symbol" not in frame.columns:
            frame["symbol"] = SYMBOL
        else:
            frame["symbol"] = frame["symbol"].fillna(SYMBOL).replace("", SYMBOL)

        frame["hidden_combined_segment_boundary"] = False
        if len(frame):
            frame.loc[frame.index[0], "hidden_combined_segment_boundary"] = True
    ...
    sort_columns = [
        column
        for column in ["timestamp", "simulation_run_id", "source_scenario", "source_seed"]
