#!/usr/bin/env python3
"""Rank weighted rawseq prediction ensembles with agreement gates.

This averages already-probed prediction columns only. It does not average model
weights, train, mutate champions, promote artifacts, or place orders.
"""
...
GATE_GRID_ENV = os.getenv(
    "RAWSEQ_ENSEMBLE_GATE_GRID",
    "none,all_positive,all_above_0,min_pred_above_-0.1,min_pred_above_0,"
    "primary_above_0.2_secondary_above_-0.1,primary_above_0.3_secondary_above_0",
)
...
OUTPUT_ROOT = Path(
    os.getenv(
        ...
        str(PROJECT_ROOT / "data" / "research" / "rawseq_probe_ensemble_gate_rankings"),
    )
)
...
def parse_gate_grid() -> list[str]:
    gates = [item.strip() for item in re.split(r"[,;\n]+", GATE_GRID_ENV) if item.strip()]
    unknown = [gate for gate in gates if gate not in VALID_GATES]
    if unknown:
        raise SystemExit(f"Unknown gate(s): {unknown}. Valid gates: {sorted(VALID_GATES)}")
...
def gate_mask(frame: pd.DataFrame, gate: str, pred_columns: list[str]) -> pd.Series:
    preds = frame[pred_columns].apply(pd.to_numeric, errors="coerce")
    if gate == "none":
        mask = pd.Series(True, index=frame.index)
    elif gate == "all_positive":
        mask = preds.gt(0.0).all(axis=1)
    elif gate == "all_above_0":
        mask = preds.ge(0.0).all(axis=1)
    elif gate == "min_pred_above_-0.1":
        mask = preds.min(axis=1) > -0.1
    elif gate == "min_pred_above_0":
        mask = preds.min(axis=1) > 0.0
    elif gate == "primary_above_0.2_secondary_above_-0.1":
        secondary_ok = preds.iloc[:, 1:].gt(-0.1).all(axis=1) if len(pred_columns) > 1 else True
        mask = preds.iloc[:, 0].gt(0.2) & secondary_ok
    elif gate == "primary_above_0.3_secondary_above_0":
        secondary_ok = preds.iloc[:, 1:].gt(0.0).all(axis=1) if len(pred_columns) > 1 else True
        mask = preds.iloc[:, 0].gt(0.3) & secondary_ok
    else:
        raise SystemExit(f"Unhandled gate: {gate}")
    return mask & preds.notna().all(axis=1)
...
def rolling_positive_fractions(selected: pd.DataFrame) -> dict[str, float]:
    ...
    if selected.empty:
        for hours in ROLLING_WINDOW_HOURS:
            ...
            output[f"rolling_{hours:g}h_worst_cum_bps"] = math.nan
        return output
...
def evaluate_combo(frame: pd.DataFrame, weights: list[float], threshold: float, cost: float, gate: str) -> dict[str, Any]:
    ...
    policy_mask, gross_series = selected_mask_and_gross(frame, threshold)
    agreement_mask = gate_mask(frame, gate, pred_columns)
    mask = policy_mask & agreement_mask
    ...
    dip_ratio = abs(max_dip) / cum_net if selected_rows and cum_net > 0.0 else math.inf
    row: dict[str, Any] = {
        "gate": gate,
...
def render_ranked_row(rank: int, row: pd.Series) -> str:
    return (
        f"  {rank:02d}. status={row['status']} score={fmt(row['score'], 3)} gate={row['gate']} "
        f"weights={row['weights']} thr={float(row['threshold_bps']):g} cost={float(row['cost_bps']):g} "
...
def render_text(ranking: pd.DataFrame, contracts: list[dict[str, Any]], warnings: list[str], output_dir: Path) -> str:
    ...
    lines = [
        "Rawseq Probe Gated Ensemble Ranking",
        "",
    ...
    lines += [
        "",
        f"Ranking CSV: {output_dir / 'gated_ensemble_ranking.csv'}",
        f"Best contract JSON: {output_dir / 'best_gated_ensemble_contract.json'}",
...
def main() -> None:
    ...
    for weights in weight_grid:
        for gate in gates:
            for threshold in thresholds:
                for cost in costs:
                    row = evaluate_combo(aligned.copy(), weights, threshold, cost, gate)
    ...
    output_dir = OUTPUT_ROOT / f"rawseq_probe_gated_ensemble_ranking_{now_stamp()}_{uuid.uuid4().hex[:8]}"
    output_dir.mkdir(parents=True, exist_ok=False)
    ranking.to_csv(output_dir / "gated_ensemble_ranking.csv", index=False)
    best_contract = make_contract(contracts, warnings, ranking.iloc[0], output_dir)
    (output_dir / "best_gated_ensemble_contract.json").write_text(
        json.dumps(best_contract, indent=2, sort_keys=True), encoding="utf-8"
    )
    text = render_text(ranking, contracts, warnings, output_dir)
    (output_dir / "gated_ensemble_ranking.txt").write_text(text, encoding="utf-8")
    print(text)
