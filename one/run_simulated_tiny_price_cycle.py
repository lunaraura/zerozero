...
TRAIN_EVERY_RUNS = max(1, env_int("SIM_ROLLING_TRAIN_EVERY_RUNS", "5"))
...
def read_run_frame(run, suffix):
    ...
    if tail_10s_rows > 0:
        if suffix == "10s_flow":
            ...
            one_minute_tail = max(1, int(tail_10s_rows / 60))
    ...
    holdout_runs = [
        {
            ...
            "ten_second_rows": archived["archived"].get("10s_flow", {}).get("rows", 0),
            "tail_10s_rows": 0,
            "mtime": time.time(),
        }
    ]
    combine = write_combined_dataset(holdout_runs, HOLDOUT_VENUE_DIR)
